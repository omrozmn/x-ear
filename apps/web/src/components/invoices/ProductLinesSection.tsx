...
import type { InventoryItem as ApiInventoryItem } from '../../api/generated/xEarCRMAPIAutoGenerated.schemas.ts/inventoryItem';
...
export function ProductLinesSection({
  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      try {
        ...
        const params = { page: 1, per_page: 100, status: 'IN_STOCK' as const };
        const authTokenPresent = !!localStorage.getItem(AUTH_TOKEN);
        console.log('🔎 Fetching inventory via API', { params, authTokenPresent });

        const resp = await api.inventoryGetInventoryItems(params);
  ...
  const handleLineChange = (index: number, field: keyof ProductLine, value: any) => {
    ...
    if (['quantity', 'unitPrice', 'discount', 'discountType', 'taxRate'].includes(field)) {
      ...
      const disc = line.discount === '' ? 0 : safeForCompute(line.discount);
      if (disc) {
  ...
  const handleProductSelect = useCallback(async (index: number, productIdOrObject: string | LocalInventoryItem) => {
    ...
    if (typeof productIdOrObject === 'string') {
      ...
      if (!product) {
        try {
          const api = getXEarCRMAPIAutoGenerated();
          const resp = await api.inventoryGetInventoryItem(productIdOrObject);
      ...
      if (product && (product.taxRate === undefined || product.taxRate === null)) {
        try {
          const api = getXEarCRMAPIAutoGenerated();
          const resp = await api.inventoryGetInventoryItem(product.id);
          const it = (resp as any)?.data?.data ?? (resp as any)?.data ?? null;
          if (it) {
  ...
  const handleCreateInventoryFromLine = async (index: number) => {
    ...
    const idempotencyKey = `${line.id}-create-${Date.now()}`;
    const parsed = parseNameBrandModel(line.name);
    const vatRateNum = typeof line.taxRate === 'string' ? parseFloat(line.taxRate) : (line.taxRate || 18);
    const body: Record<string, any> = {
      name: parsed.name || line.name,
    ...
    try {
      console.log('🆕 Envanter’e ekleme isteği', body);
      const resp = await api.inventoryCreateInventoryItem(body, {
  ...
  const openWithholdingModal = (index: number) => {
    if (typeof ({} as ProductLinesSectionProps).onRequestLineEditor !== 'undefined') {
      // placeholder check - actual prop exists via closure; we'll use the prop below
    }
    if (typeof ({} as any).onRequestLineEditor !== 'undefined') {
      // no-op
    }
    if ((arguments && (arguments as any).callee) || true) {
      // prefer prop callback when provided via closure
    }
    if (typeof ({} as any) === 'object') {
      // fallback
    }
    // If parent supplied an onRequestLineEditor prop, prefer that to opening a modal
    // (we check the actual prop value below via closure)
              ...
              <div className="grid grid-cols-12 gap-3">
                ...
                <div className="col-span-2">
                  <Input
                    type="number"
                    label="Miktar"
                    value={line.quantity === '' || line.quantity === undefined ? '' : String(line.quantity)}
