...
interface PartyAdvancedSearchProps {
  parties: Party[];
  onFilteredResults: (results: Party[]) => void;
  onClearFilters: () => void;
}
...
export const PartyAdvancedSearch: React.FC<PartyAdvancedSearchProps> = ({
  parties,
  onFilteredResults,
  onClearFilters
}) => {
  ...
  const filteredResults = useMemo(() => {
    ...
    if (filters.query.trim()) {
      if (filters.fuzzySearch) {
        ...
        results = results.filter(party => {
          const address = typeof party.address === 'string' ? party.address : party.addressFull;
          return party.firstName?.toLowerCase().includes(query) ||
            party.lastName?.toLowerCase().includes(query) ||
    ...
    if (filters.ageRange.min !== null || filters.ageRange.max !== null) {
      results = results.filter(party => {
        if (!party.birthDate) return false;
    ...
    // Segment
    if (filters.segment) {
      results = results.filter(party => party.segment === filters.segment);
    }

    // Durum
    if (filters.status) {
      results = results.filter(party => party.status === filters.status);
    }

    // Telefon varlığı
    if (filters.hasPhone !== null) {
      results = results.filter(party =>
        filters.hasPhone ? !!party.phone : !party.phone
      );
    }
    ...
    if (filters.hasEmail !== null) {
      results = results.filter(party =>
        filters.hasEmail ? !!party.email : !party.email
      );
    }
    ...
    if (filters.hasNotes !== null) {
      results = results.filter(party =>
        filters.hasNotes ? (party.notes && party.notes.length > 0) : (!party.notes || party.notes.length === 0)
      );
    }
    ...
    results.sort((a, b) => {
      ...
      if (filters.sortBy === 'name') {
        ...
        aValue = a[filters.sortBy as keyof Party] as string | number | Date | undefined;
        bValue = b[filters.sortBy as keyof Party] as string | number | Date | undefined;
      }
...
export default PartyAdvancedSearch;