  ...
  const sgkColumns: Column<DeliveredEReceipt>[] = [
    {
      key: 'number',
      title: 'E-Reçete No',
      render: (_, r) => <span className="text-sm font-medium text-foreground">{r.number}</span>,
    },
    {
      key: 'date',
      title: 'Tarih',
      render: (_, r) => <span className="text-sm text-muted-foreground">{new Date(r.date).toLocaleDateString('tr-TR')}</span>,
    },
    {
      key: 'doctorName',
      title: 'Doktor',
      render: (_, r) => <span className="text-sm text-muted-foreground">{r.doctorName}</span>,
    },
    {
      key: '_materials',
      title: 'Malzeme Sayısı',
      render: (_, r) => <span className="text-sm text-muted-foreground">{r.materials.length}</span>,
    },
    {
      ...
      render: (_, r) => (
        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
          r.sgkDocumentAvailable ? 'bg-success/10 text-success' : 'bg-warning/10 text-yellow-800'
        }`}>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-foreground">{selectedEReceipt.number}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground">Tarih</label>
                    <p className="text-sm text-foreground">{new Date(selectedEReceipt.date).toLocaleDateString('tr-TR')}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground">Doktor</label>
                    <p className="text-sm text-foreground">{selectedEReceipt.doctorName}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground">Geçerlilik</label>
                    <p className="text-sm text-foreground">{new Date(selectedEReceipt.validUntil).toLocaleDateString('tr-TR')}</p>
                  </div>
                </div>

                {/* SGK Belge Durumu */}
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">SGK Belge Durumu</label>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {selectedEReceipt.materials.map((material, index) => (
                      <div key={`${material.code}-${index}`} className="border rounded-2xl p-4 bg-success/10 border-green-200">
