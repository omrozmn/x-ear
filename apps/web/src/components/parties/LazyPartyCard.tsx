import React, { memo, useCallback, useRef, useEffect, useMemo } from 'react';
import { Button, Badge } from '@x-ear/ui-web';
import {
  User,
  Phone,
  Mail,
  MessageSquare,
  Edit,
  Trash2,
  MapPin,
  Clock
} from 'lucide-react';
import { Party } from '../../types/party';
import { useLazyLoading } from '../../hooks/useVirtualScroll';

interface LazyPartyCardProps {
  party: Party;
  isSelected?: boolean;
  onSelect?: (partyId: string) => void;
  onClick?: (party: Party) => void;
  onEdit?: (party: Party) => void;
  onDelete?: (party: Party) => void;
  onCommunication?: (party: Party) => void;
  variant?: 'compact' | 'detailed';
  showActions?: boolean;
  className?: string;
}

const LazyPartyCard: React.FC<LazyPartyCardProps> = memo(({
  party,
  isSelected = false,
  onSelect,
  onClick,
  onEdit,
  onDelete,
  onCommunication,
  variant = 'compact',
  showActions = true,
  className = ''
}) => {
  const cardRef = useRef<HTMLDivElement>(null);

  const { observerRef, isVisible } = useLazyLoading({
    threshold: 0.1,
    rootMargin: '50px'
  });

  // Set up intersection observer
  useEffect(() => {
    if (cardRef.current && party.id) {
      const cleanup = observerRef(cardRef.current, party.id);
      return cleanup;
    }
  }, [observerRef, party.id]);

  // Format phone number
  const formatPhone = useCallback((phone?: string) => {
    if (!phone) return '-';
    const cleaned = phone.replace(/\D/g, '');
    if (cleaned.length === 11 && cleaned.startsWith('0')) {
      return `${cleaned.slice(0, 4)} ${cleaned.slice(4, 7)} ${cleaned.slice(7, 9)} ${cleaned.slice(9)}`;
    }
    return phone;
  }, []);

  // Get party display name
  const displayName = useMemo(() => {
    if (party.firstName && party.lastName) {
      return `${party.firstName} ${party.lastName}`;
    }
    return `${party.firstName || ''} ${party.lastName || ''}`.trim() || 'İsimsiz Hasta';
  }, [party.firstName, party.lastName]);

  // Get status badge
  const getStatusBadge = useCallback((status?: string) => {
    switch (status) {
      case 'active':
        return <Badge variant="success" size="sm">Aktif</Badge>;
      case 'inactive':
        return <Badge variant="warning" size="sm">Pasif</Badge>;
      case 'archived':
        return <Badge variant="secondary" size="sm">Arşiv</Badge>;
      default:
        return <Badge variant="default" size="sm">{status}</Badge>;
    }
  }, []);

  // Get label badge
  const getLabelBadge = useCallback((label?: string) => {
    switch (label) {
      case 'yeni':
        return <Badge variant="primary" size="sm">Yeni</Badge>;
      case 'arama-bekliyor':
        return <Badge variant="warning" size="sm">Arama Bekliyor</Badge>;
      case 'randevu-verildi':
        return <Badge variant="primary" size="sm">Randevu Verildi</Badge>;
      case 'deneme-yapildi':
        return <Badge variant="success" size="sm">Deneme Yapıldı</Badge>;
      case 'kontrol-hastasi':
        return <Badge variant="secondary" size="sm">Kontrol Hastası</Badge>;
      case 'satis-tamamlandi':
        return <Badge variant="success" size="sm">Satış Tamamlandı</Badge>;
      default:
        return <Badge variant="default" size="sm">{label}</Badge>;
    }
  }, []);

  // Handle card click
  const handleCardClick = useCallback(() => {
    if (onClick) {
      onClick(party);
    } else if (onSelect) {
      onSelect(party.id || '');
    }
  }, [onClick, onSelect, party]);

  // Handle action clicks
  const handleEdit = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    onEdit?.(party);
  }, [onEdit, party]);

  const handleDelete = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    onDelete?.(party);
  }, [onDelete, party]);

  const handleCommunication = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    onCommunication?.(party);
  }, [onCommunication, party]);

  // Show loading skeleton if not visible
  if (!isVisible) {
    return (
      <div
        ref={cardRef}
        className={`bg-white dark:bg-gray-800 rounded-2xl border border-border p-4 animate-pulse ${className}`}
      >
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 bg-accent rounded-full"></div>
          <div className="flex-1 space-y-2">
            <div className="h-4 bg-accent rounded w-3/4"></div>
            <div className="h-3 bg-accent rounded w-1/2"></div>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div
      ref={cardRef}
      className={`
        bg-white dark:bg-gray-800 rounded-2xl border border-border p-4 transition-all duration-200
        hover:shadow-md hover:border-blue-300 dark:hover:border-blue-700 cursor-pointer
        ${isSelected ? 'ring-2 ring-blue-500 border-blue-500' : ''}
        ${className}
      `}
      onClick={handleCardClick}
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-3 flex-1 min-w-0">
          {/* Avatar */}
          <div className="flex-shrink-0">
            <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center">
              <User className="w-5 h-5 text-primary" />
            </div>
          </div>

          {/* Party Info */}
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2 mb-1">
              <h3 className="text-sm font-medium text-gray-900 dark:text-white truncate">
                {displayName}
              </h3>
              {getStatusBadge(party.status || undefined)}
            </div>

            <div className="flex items-center space-x-4 text-xs text-muted-foreground">
              {party.phone && (
                <div className="flex items-center">
                  <Phone className="w-3 h-3 mr-1" />
                  <span>{formatPhone(party.phone)}</span>
                </div>
              )}
              {party.email && (
                <div className="flex items-center">
                  <Mail className="w-3 h-3 mr-1" />
                  <span className="truncate">{party.email}</span>
                </div>
              )}
            </div>

            {/* Labels and Tags */}
            <div className="flex items-center space-x-2 mt-2">
              {getLabelBadge(party.label)}
              {party.tags && party.tags.length > 0 && (
                <div className="flex space-x-1">
                  {party.tags.slice(0, 2).map((tag, index) => (
                    <span
                      key={index}
                      className="px-2 py-1 text-xs bg-muted text-muted-foreground rounded"
                    >
                      {tag}
                    </span>
                  ))}
                  {party.tags.length > 2 && (
                    <span className="text-xs text-muted-foreground">
                      +{party.tags.length - 2}
                    </span>
                  )}
                </div>
              )}
            </div>
          </div>
        </div>

        {/* Action Buttons */}
        {showActions && (
          <div className="flex items-center space-x-1 ml-2">
            {onCommunication && (
              <Button
                variant="ghost"
                size="sm"
                onClick={handleCommunication}
                className="p-1 h-8 w-8"
              >
                <MessageSquare className="w-4 h-4" />
              </Button>
            )}
            {onEdit && (
              <Button
                variant="ghost"
                size="sm"
                onClick={handleEdit}
                className="p-1 h-8 w-8"
              >
                <Edit className="w-4 h-4" />
              </Button>
            )}
            {onDelete && (
              <Button
                variant="ghost"
                size="sm"
                onClick={handleDelete}
                className="p-1 h-8 w-8 text-destructive hover:text-destructive dark:hover:text-red-300"
              >
                <Trash2 className="w-4 h-4" />
              </Button>
            )}
          </div>
        )}
      </div>

      {/* Additional Info for Detailed Variant */}
      {variant === 'detailed' && (
        <div className="mt-3 pt-3 border-t border-border">
          <div className="space-y-2">
            {(party.addressFull || party.address) && (
              <div className="flex items-center text-xs text-muted-foreground">
                <MapPin className="w-3 h-3 mr-1" />
                <span className="truncate">
                  <span className="truncate">
                    {(() => {
                      const address = party.address;
                      if (typeof address === 'object' && address !== null) {
                        const addressObj = address as {
                          street?: string;
                          city?: string;
                          district?: string;
                          postalCode?: string;
                          country?: string;
                          full?: string;
                        };
                        return addressObj.full ||
                          `${addressObj.district || ''} ${addressObj.city || ''}`.trim() ||
                          'Adres bilgisi yok';
                      }
                      return party.addressFull || address || 'Adres bilgisi yok';
                    })()}
                  </span>
                </span>
              </div>
            )}

            {party.devices && party.devices.length > 0 && (
              <div className="flex items-center justify-between">
                <span className="text-xs text-success font-medium">
                  {party.devices.length} cihaz
                </span>
                {party.lastContactDate && (
                  <div className="flex items-center text-xs text-muted-foreground">
                    <Clock className="w-3 h-3 mr-1" />
                    Son iletişim: {new Date(party.lastContactDate).toLocaleDateString('tr-TR')}
                  </div>
                )}
              </div>
            )}

            {party.notes && party.notes.length > 0 && (
              <p className="text-xs text-muted-foreground line-clamp-2">
                {party.notes[0].text}
              </p>
            )}
          </div>
        </div>
      )}
    </div>
  );
});

LazyPartyCard.displayName = 'LazyPartyCard';

export default LazyPartyCard;