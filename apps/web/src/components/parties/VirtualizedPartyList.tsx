/**
 * Virtualized Party List Component
...
interface PartyRowProps {
  ...
  data: {
    ...
    onEdit?: (party: Party) => void;
    onDelete?: (party: Party) => void;
    onCommunicationClick?: (party: Party) => void;
  };
}


// Party row component for virtualization
const PartyRow = ({ index, style, data }: PartyRowProps) => {
  const party = data.parties[index];
  const isSelected = data.selectedParties.includes(party.id || '');
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 mr-3">
        <Checkbox
          checked={isSelected}
          onChange={() => data.onPartySelect?.(party.id || '')}
        />
      </div>
      ...
      <div className="flex-shrink-0 flex items-center space-x-1">
        ...
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation();
            data.onCommunicationClick?.(party);
          }}
          title="İletişim"
        >
...
export const VirtualizedPartyList: React.FC<VirtualizedPartyListProps> = ({
  parties,
  loading = false,
  selectedParties = [],
  onPartySelect,
...
export default VirtualizedPartyList;