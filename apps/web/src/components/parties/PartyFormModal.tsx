...
// import { Party, PartyStatus } from '../../types/party'; // Removed in favor of generated types
...
interface PartyFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: PartyCreate) => Promise<PartyRead | null>;
  initialData?: PartyRead | null;
  title?: string;
  className?: string;
  isLoading?: boolean;
}
...
/**
 * PartyFormModal Component
 * Modal form for creating and editing parties
...
export function PartyFormModal({
  ...
  const handleInputChange = useCallback((field: keyof PartyFormData, value: string) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
