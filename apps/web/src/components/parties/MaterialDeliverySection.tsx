...
export const MaterialDeliverySection: React.FC<MaterialDeliverySectionProps> = ({ partyId }) => {
  ...
  const handleCreateDelivery = async () => {
    ...
    try {
      ...
      // Log to timeline
      try {
        await timelineService.addDeviceEvent(partyId, 'delivery_created', {
  ...
  const handleMarkAsDelivered = async (batchId: string) => {
    try {
      ...
      if (batch) {
        await timelineService.addDeviceEvent(partyId, 'delivery_completed', {
