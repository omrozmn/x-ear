...
export const PartyOfflineStatus: React.FC<PartyOfflineStatusProps> = ({
  ...
  const getStatusColor = () => {
    if (!syncStatus.isOnline) return 'bg-destructive/10 text-red-800 border-red-200';
    if (syncStatus.isSyncing) return 'bg-primary/10 text-blue-800 border-blue-200';
    if (syncStatus.pendingOperations > 0) return 'bg-warning/10 text-yellow-800 border-yellow-200';
  ...
  return (
    <div className={`flex items-center gap-3 p-3 bg-card border rounded-2xl shadow-sm ${className}`}>
      {/* Status Badge */}
