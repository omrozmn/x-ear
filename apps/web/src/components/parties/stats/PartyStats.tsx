...
export const PartyStats: React.FC<PartyStatsProps> = ({
  totalParties = 0,
  activeParties = 0,
  inactiveParties = 0,
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        console.error('Failed to refresh party stats:', error)
      } finally {
  ...
  const stats: StatItem[] = useMemo(() => {
    const baseStats: StatItem[] = [
      ...
      },
      {
        id: 'new',
        name: 'Bu Ay Yeni',
        value: newPartiesThisMonth,
        icon: TrendingUp,
        color: 'text-purple-600',
        bgColor: 'bg-purple-100',
        trend: showTrends ? calculateTrend(newPartiesThisMonth, Math.floor(newPartiesThisMonth * 0.8)) : undefined,
    ...
    <div className={className}>
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-${showExtendedStats ? '4' : '4'} gap-4`}>
        {stats.map((stat) => {
          ...
          const isNegativeTrend = stat.id === 'inactive' // Inactive parties going up is bad
