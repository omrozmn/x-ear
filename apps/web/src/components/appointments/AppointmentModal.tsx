...
export const AppointmentModal: React.FC<AppointmentModalProps> = ({
  ...
  const getStatusColor = (status: AppointmentStatus): string => {
    const colors = {
      scheduled: 'bg-primary/10 text-blue-800',
      confirmed: 'bg-success/10 text-success',
      completed: 'bg-muted text-foreground',
      cancelled: 'bg-destructive/10 text-red-800',
      no_show: 'bg-orange-100 text-orange-800',
      rescheduled: 'bg-warning/10 text-yellow-800'
    };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden" data-testid="appointment-modal">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl text-left shadow-xl transform transition-all sm:max-w-2xl sm:w-full max-h-[90vh] flex flex-col mx-4">
        {/* Header */}
        <div className="bg-white dark:bg-slate-800 px-4 py-3 border-b border-border dark:border-slate-700 flex-shrink-0">
          ...
          {currentMode === 'view' && appointment ? (
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-slate-700 rounded-2xl p-4">
                ...
                <p className="text-sm text-muted-foreground">ID: {appointment.partyId}</p>
              </div>

              {/* Appointment Details */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground">{t('modal.date')}</label>
                  <p className="mt-1 text-sm text-gray-900 dark:text-white">{formatDate(appointment.date)}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground">{t('modal.time')}</label>
                  <p className="mt-1 text-sm text-gray-900 dark:text-white">{formatTime(appointment.time)}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground">{t('modal.duration')}</label>
                  <p className="mt-1 text-sm text-gray-900 dark:text-white">{appointment.duration} dakika</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground">{t('modal.type')}</label>
                  <p className="mt-1 text-sm text-gray-900 dark:text-white">{getTypeLabel(appointment.type)}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground">{t('modal.status')}</label>
                ...
                {appointment.clinician && (
                  <div>
                    <label className="block text-sm font-medium text-foreground">{t('modal.doctor')}</label>
                    <p className="mt-1 text-sm text-gray-900 dark:text-white">{appointment.clinician}</p>
                  </div>
                )}
                {appointment.location && (
                  <div>
                    <label className="block text-sm font-medium text-foreground">{t('modal.location')}</label>
              ...
              {/* Title */}
              <div>
                <label className="block text-sm font-medium text-foreground">{t('modal.title')}</label>
              ...
              {appointment.notes && (
                <div>
                  <label className="block text-sm font-medium text-foreground">{t('modal.notes')}</label>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {appointment.status !== 'confirmed' && (
                      <Button
                        ...
                        disabled={updating}
                        className="px-3 py-1 text-xs font-medium text-success bg-success/10 rounded-full hover:bg-green-200 dark:hover:bg-green-900/50 disabled:opacity-50"
                        variant='default'>
                    ...
                    <Button
                      ...
                      disabled={updating}
                      className="px-3 py-1 text-xs font-medium text-foreground bg-muted rounded-full hover:bg-accent dark:hover:bg-gray-600 disabled:opacity-50"
                      variant='default'>
                    ...
                    <Button
                      ...
                      disabled={updating}
                      className="px-3 py-1 text-xs font-medium text-yellow-700 bg-warning/10 dark:text-yellow-300 rounded-full hover:bg-yellow-200 dark:hover:bg-yellow-900/50 disabled:opacity-50"
                      variant='default'>
        ...
        {currentMode === 'view' && appointment && (
          <div className="bg-gray-50 dark:bg-slate-700 px-4 py-3 sm:px-6 flex items-center justify-between border-t border-border dark:border-slate-600 flex-shrink-0">
            {/* Delete button on the left */}
            <Button
              onClick={() => setShowDeleteConfirm(true)}
              className="text-destructive hover:bg-destructive/10 dark:hover:bg-red-900/20"
              variant='outline'>
            ...
            <div className="inline-block align-bottom bg-card rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div className="mt-2">
                      ...
                      {appointment && (
                        <div className="mt-3 p-3 bg-muted rounded-xl">
                          <p className="text-sm font-medium text-foreground">{appointment.partyName}</p>
              ...
              <div className="bg-muted px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                ...
                <Button
                  ...
                  disabled={deleting}
                  className="mt-3 w-full inline-flex justify-center rounded-xl border border-border shadow-sm px-4 py-2 bg-card text-base font-medium text-foreground hover:bg-muted focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm disabled:opacity-50"
                  variant='default'>
                  {t('cancel', { ns: 'common' })}
                </Button>
              </div>
