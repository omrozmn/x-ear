...
export const OutboxStatus: React.FC<OutboxStatusProps> = ({ className = '' }) => {
  ...
  return (
    <div className={`outbox-status ${className}`}>
      ...
      <div className={`flex items-center gap-2 ${isOnline ? 'text-success' : 'text-destructive'}`}>
      ...
      {hasFailedOperations && (
        <div className="flex items-center gap-2 text-destructive">
          ...
          <Button
            ...
            className="text-xs px-2 py-1 bg-destructive/10 text-destructive rounded hover:bg-red-200 transition-colors"
            variant='default'>
            Clear
          </Button>
        </div>
      ...
      {isOnline && hasPendingOperations && !isSyncing && (
        <Button
          onClick={syncNow}
          className="flex items-center gap-1 text-sm px-3 py-1 bg-primary/10 text-primary rounded hover:bg-blue-200 transition-colors"
          variant='default'>
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
