        ...
        <div className="fixed inset-0 z-[60] overflow-hidden">
            ...
            <div className="absolute inset-y-0 right-0 max-w-full flex">
                <div className={`w-screen max-w-md transform transition-transform duration-300 ease-in-out shadow-2xl ${aiInboxOpen ? 'translate-x-0' : 'translate-x-full'
                    ...
                    <div className="h-full flex flex-col bg-card overflow-hidden relative rounded-l-3xl">
                        {/* Close Button Inside Drawer (for mobile) */}
                        <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => setAiInboxOpen(false)}
                            className="absolute top-4 right-4 p-2 rounded-full hover:bg-muted text-muted-foreground z-20 h-auto"
                        >
                            <X size={20} />
                        </Button>
