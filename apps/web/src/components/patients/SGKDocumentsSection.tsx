...
interface DeliveredEReceipt {
  ...
  patientFormAvailable: boolean; // Hasta işlem formu indirilebilir durumda mı
}

interface SGKDocumentsSectionProps {
  deliveredEReceipts: DeliveredEReceipt[];
  onDownloadPatientForm?: (eReceiptId: string) => void;
  onDownloadAllPatientForms?: (selectedMonth?: string) => void;
  onError?: (message: string) => void;
}

export const SGKDocumentsSection: React.FC<SGKDocumentsSectionProps> = ({
  deliveredEReceipts,
  onDownloadPatientForm,
  onDownloadAllPatientForms,
  onError
}) => {
  ...
  const handleDownloadPatientForm = (eReceiptId: string) => {
    if (onDownloadPatientForm) {
      onDownloadPatientForm(eReceiptId);
    } else {
      if (onError) {
  ...
  const handleDownloadAllPatientForms = () => {
    if (onDownloadAllPatientForms) {
      onDownloadAllPatientForms(selectedMonth);
    } else {
      if (onError) {
      ...
      <div className="bg-white rounded-lg border p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button
              onClick={handleDownloadAllPatientForms}
                ...
                {filteredEReceipts.map((receipt) => (
                  <tr key={receipt.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      ...
                      {receipt.patientFormAvailable && (
                        <Button
                          size="sm"
                          onClick={() => handleDownloadPatientForm(receipt.id)}
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t">
                  {selectedEReceipt.patientFormAvailable && (
                    <Button
                      onClick={() => handleDownloadPatientForm(selectedEReceipt.id)}
                      className="bg-green-600 hover:bg-green-700 text-white"
                    >
                      <Download className="w-4 h-4 mr-2" />
