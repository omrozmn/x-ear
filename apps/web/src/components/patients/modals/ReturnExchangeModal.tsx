...
import { Sale as APISale, SalesUpdateSaleBody } from '../../../api/generated/xEarCRMAPIAutoGenerated.schemas';
...
export const ReturnExchangeModal: React.FC<ReturnExchangeModalProps> = ({
  ...
  // Fuzzy search utility for product selection
  ...
  const createReturnInvoiceForReturn = async (data: ReturnExchangeData) => {
    ...
    try {
      // Use real API to create return invoice
      const api = getXEarCRMAPIAutoGenerated();
      
      // Create return invoice via API
      const invoiceResponse = await api.replacementsCreateReturnInvoice(replacementId || `REPL-${Date.now()}`, {
      ...
      if (invoiceResponse.data?.success) {
        ...
        throw new Error(invoiceResponse.data?.error || 'İade faturası oluşturulamadı');
      }
  ...
  const sendInvoiceToGib = async (invoiceId: string) => {
    ...
    try {
      ...
      // Send invoice to GIB via API
      const gibResponse = await api.replacementsSendInvoiceToGib(invoiceId);
      
      if (gibResponse.data?.success) {
        const invoiceData = gibResponse.data.data;
        ...
        throw new Error(gibResponse.data?.error || 'GİB gönderimi başarısız');
      }
