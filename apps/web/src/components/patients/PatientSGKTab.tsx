...
import { Button, Input, Badge, Card, CardContent, CardHeader, CardTitle } from '@x-ear/ui-web';
import { useToastHelpers } from '@x-ear/ui-web';
import { Search, FileText, Download, Eye, RefreshCw, Upload, Trash2, AlertCircle, CheckCircle, Clock, Plus, X } from 'lucide-react';
import { Patient } from '@/types/patient';
import { SGKDocument, SGKDocumentType, SGKWorkflowStatus } from '@/types/sgk';
...
export const PatientSGKTab: React.FC<PatientSGKTabProps> = ({ patient, onPatientUpdate }) => {
  ...
  // SGK hooks
  const processOcrMutation = useProcessSgkOcr();
  const processingMutation = useTriggerSgkProcessing();

  const sgkData = patient.sgkInfo || {};
  const sgkStatus = patient.sgkStatus || 'pending';
  ...
  const sgkCoverage = Math.round((sgkData.coveragePercentage || 85) * 100); // Convert percentage to amount
