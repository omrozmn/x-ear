/**
 * Patient Offline Status Component
...
interface PatientOfflineStatusProps {
  syncStatus: {
    isOnline: boolean;
    isSyncing: boolean;
...
export const PatientOfflineStatus: React.FC<PatientOfflineStatusProps> = ({
  syncStatus,
  onSync,
  className = ''
}) => {
    ...
    <div className={`flex items-center gap-3 p-3 bg-white border rounded-lg shadow-sm ${className}`}>
      ...
      <div className="flex items-center gap-4 text-sm text-gray-600">
        <div className="flex items-center gap-1">
          <span className="font-medium">{syncStatus.totalPatients}</span>
          <span>hasta</span>
        </div>
...
export default PatientOfflineStatus;