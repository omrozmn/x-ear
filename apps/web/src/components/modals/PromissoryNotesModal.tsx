...
export const PromissoryNotesModal: React.FC<PromissoryNotesModalProps> = ({
  ...
  const fetchPromissoryNotes = async () => {
    ...
    setError(null);
    try {
      // TODO: Implement API call to fetch promissory notes
      const response = await fetch(`/api/sales/${sale.id}/promissory-notes`);
      if (!response.ok) throw new Error('Senetler yüklenemedi');
