...
interface DeviceEditModalProps {
  ...
  onClose: () => void;
  onSave: (device: PartyDevice) => void;
}
...
interface ExtendedFormData extends Partial<PartyDevice> {
...
export const DeviceEditModal: React.FC<DeviceEditModalProps> = ({
  ...
  useEffect(() => {
    if (device) {
      setFormData({
        ...
        partyPayment: device.partyPayment || 0,
  ...
  const handleDeviceSelect = (device: DeviceInventoryItem) => {
    ...
    setFormData(prev => ({
      ...
      partyPayment: Math.max(0, device.listPrice - Math.min(sgkAmount, device.listPrice))
    }));
  };
  ...
  const handleInputChange = (field: keyof ExtendedFormData, value: unknown) => {
    ...
    if (field === 'listPrice' || field === 'discountValue' || field === 'discountType') {
      ...
      setFormData(prev => ({
        ...
        partyPayment: Math.max(0, newSalePrice - (prev.sgkReduction || 0))
      }));
    }
    ...
    if (field === 'sgkReduction') {
      ...
      setFormData(prev => ({
        ...prev,
        partyPayment: Math.max(0, salePrice - numericValue)
      }));
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const deviceData: PartyDevice = {
      ...
      partyPayment: formData.partyPayment,
        ...
        <div className="space-y-4">
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <p className="font-medium text-foreground">{formatCurrencyTR(device.listPrice)}</p>
          ...
          {selectedDevice && (
            <div className="bg-primary/10 border border-blue-200 rounded-2xl p-4">
              ...
              <p className="text-sm text-primary">{formatCurrencyTR(selectedDevice.listPrice)}</p>
            </div>
          )}
