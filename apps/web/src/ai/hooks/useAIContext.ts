...
/**
 * Map user role string to AIRole type
 * 
 * @param userRole - Role string from auth store
 * @param isSuperAdmin - Whether user is super admin
 * @returns Mapped AIRole
 */
function mapUserRole(userRole: string | undefined, isSuperAdmin: boolean | undefined): AIRole {
  ...
  switch (userRole?.toLowerCase()) {
    ...
    case 'party':
      return 'PARTY';
    default:
      return 'STAFF'; // Default to STAFF for unknown roles
  }
}

/**
 * Extract tenant_id from various sources
 * Priority: effectiveTenantId > localStorage > JWT decode
 * 
 * @param user - User object from auth store
 * @returns Tenant ID or null
 */
function extractTenantId(user: { effectiveTenantId?: string } | null): string | null {
  // 1. Check effectiveTenantId from user (set during tenant impersonation)
  ...
  const contextData = useMemo(() => {
    ...
    if (!tenantId) {
      return {
        ...
        tenantId: null,
        partyId: null,
        role: mapUserRole(user.role, user.is_super_admin),
      };
    }
    ...
    // Derive role
    const role = mapUserRole(user.role, user.is_super_admin);
