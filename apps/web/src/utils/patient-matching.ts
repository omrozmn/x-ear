...
export class PatientMatcher {
   ...
   */
  public findAllMatches(patients: Patient[]): PatientMatch[] {
    const matches: PatientMatch[] = [];
    const processedPairs = new Set<string>();

    for (let i = 0; i < patients.length; i++) {
      for (let j = i + 1; j < patients.length; j++) {
        const patient1 = patients[i];
        const patient2 = patients[j];
        const pairKey = `${Math.min(i, j)}-${Math.max(i, j)}`;

        if (processedPairs.has(pairKey)) continue;
        processedPairs.add(pairKey);

        const match = this.comparePatients(patient1, patient2);
   ...
   */
  public comparePatients(patient1: Patient, patient2: Patient): Omit<PatientMatch, 'id' | 'patients'> | null {
    let totalScore = 0;
    let totalWeight = 0;
    ...
    if (patient1.tcNumber && patient2.tcNumber) {
      if (patient1.tcNumber === patient2.tcNumber) {
    ...
    if (patient1.phone && patient2.phone) {
      const phoneScore = this.comparePhones(patient1.phone, patient2.phone);
    ...
    if (patient1.email && patient2.email) {
      const emailScore = this.compareEmails(patient1.email, patient2.email);
   ...
   */
  public findMatchesForPatient(targetPatient: Patient, patients: Patient[]): PatientMatch[] {
    const matches: PatientMatch[] = [];
    ...
    for (let i = 0; i < patients.length; i++) {
      ...
      const match = this.comparePatients(targetPatient, patient);
      if (match && match.similarity >= this.config.thresholds.low) {
        matches.push({
          ...match,
          id: `match-${targetPatient.id}-${patient.id}`,
          patients: [targetPatient, patient]
        });
      }
    }
...
// Singleton instance
export const patientMatcher = new PatientMatcher();
