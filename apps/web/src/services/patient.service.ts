...
export class PatientService {
  ...
  async createPatient(patientData: Omit<Patient, 'id' | 'createdAt' | 'updatedAt'>): Promise<Patient> {
    try {
      ...
      const api = getXEarCRMAPIAutoGenerated();
      const response = await api.patientsCreatePatient(patientData as any);
      
      if (!response.data?.success || !response.data?.data) {
        throw new Error(response.data?.error || 'Failed to create patient on server');
      }

      const createdPatient = convertOrvalToLegacyPatient(response.data.data);
  ...
  async updatePatient(id: string, updates: Partial<Patient>): Promise<Patient | null> {
    try {
      ...
      const api = getXEarCRMAPIAutoGenerated();
      const response = await api.patientsUpdatePatient(id, updates as any);
      
      if (!response.data?.success || !response.data?.data) {
        throw new Error(response.data?.error || 'Failed to update patient on server');
      }

      const updatedPatient = convertOrvalToLegacyPatient(response.data.data);
  ...
  async deletePatient(id: string): Promise<boolean> {
    try {
      ...
      const api = getXEarCRMAPIAutoGenerated();
      const response = await api.patientsDeletePatient(id);
      
      if (!response.data?.success) {
        throw new Error(response.data?.error || 'Failed to delete patient from server');
      }
