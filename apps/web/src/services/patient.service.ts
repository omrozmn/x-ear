...
export class PatientService {
  ...
  private async loadPatients(): Promise<void> {
    ...
    // If no cache, load from API — but only if an auth token exists to avoid spamming backend with unauthenticated requests
    try {
      const token = (typeof window !== 'undefined') ? (window as any).__AUTH_TOKEN__ || localStorage.getItem('x-ear.auth.token@v1') || localStorage.getItem('auth_token') : null;
      if (token) {
        await this.refreshFromAPI();
      } else {
        console.log('No auth token present; skipping API patient refresh until login to avoid 401s');
      }
    } catch (err) {
      // If storage access fails, attempt refresh but tolerate failures
      try { await this.refreshFromAPI(); } catch (e) { console.warn('Patient API refresh skipped due to missing token and storage access error', e); }
    }
  }
  ...
  async createPatient(patientData: Omit<Patient, 'id' | 'createdAt' | 'updatedAt'>): Promise<Patient> {
    try {
      ...
      const response = await patientsCreatePatient(patientData as any);
      const responseData = response.data as any;

      if (!responseData?.success || !responseData?.data) {
        throw new Error(responseData?.error || 'Failed to create patient on server');
      }

      const createdPatient = convertOrvalToLegacyPatient(responseData.data);
  ...
  async updatePatient(id: string, updates: Partial<Patient>): Promise<Patient | null> {
    try {
      ...
      const response = await patientsUpdatePatient(id, updates as any);
      const responseData = response.data as any;

      if (!responseData?.success || !responseData?.data) {
        throw new Error(responseData?.error || 'Failed to update patient on server');
      }

      const updatedPatient = convertOrvalToLegacyPatient(responseData.data);
  ...
  async deletePatient(id: string): Promise<boolean> {
    try {
      ...
      if (!responseData?.success) {
        throw new Error(responseData?.error || 'Failed to delete patient from server');
      }
