...
export class EFaturaXMLService {
  ...
  private convertInvoiceToEFatura(invoice: Invoice): EFaturaXMLData {
    ...
    const efaturaInvoice: EFaturaInvoice = {
      ...
      lineCountNumeric: invoice.items.length
    };
    ...
    const customer: EFaturaParty = {
      partyIdentification: {
        id: invoice.billingAddress.taxNumber || invoice.patientTcNumber || '11111111111',
        schemeID: invoice.billingAddress.taxNumber ? 'VKN' : 'TCKN'
      },
      partyName: {
        name: invoice.billingAddress.name || invoice.patientName
      },
      postalAddress: {
        streetName: invoice.billingAddress.address,
        cityName: invoice.billingAddress.city,
        postalZone: invoice.billingAddress.postalCode,
        country: {
          identificationCode: 'TR',
          name: 'Türkiye'
        }
      ...
      contact: {
        telephone: invoice.patientPhone,
        electronicMail: undefined // Add email field to invoice if needed
      }
    };

    // Convert invoice lines
    const invoiceLines: EFaturaInvoiceLine[] = invoice.items.map((item, index) => ({
      id: (index + 1).toString(),
      invoicedQuantity: {
      ...
      lineExtensionAmount: {
        ...
        value: item.totalPrice - item.taxAmount
      },
      taxTotal: {
        taxAmount: {
          currencyID: invoice.currency || 'TRY',
    ...
    const taxTotals: EFaturaTaxTotal[] = [{
      ...
      taxSubtotals: invoice.taxes.map(tax => ({
        taxableAmount: {
          currencyID: invoice.currency || 'TRY',
          value: tax.baseAmount
        },
        taxAmount: {
          currencyID: invoice.currency || 'TRY',
          value: tax.taxAmount
        ...
        taxCategory: {
          taxScheme: {
            name: tax.type.toUpperCase(),
            taxTypeCode: EFATURA_TAX_SCHEME_CODES[tax.type.toUpperCase() as keyof typeof EFATURA_TAX_SCHEME_CODES] || EFATURA_TAX_SCHEME_CODES.KDV
          }
        }
      }))
    }];
    ...
    const legalMonetaryTotal: EFaturaLegalMonetaryTotal = {
      ...
      taxExclusiveAmount: {
        currencyID: invoice.currency || 'TRY',
        value: invoice.subtotal - invoice.totalDiscount
      },
      taxInclusiveAmount: {
        currencyID: invoice.currency || 'TRY',
        value: invoice.grandTotal
      },
      allowanceTotalAmount: invoice.totalDiscount > 0 ? {
        currencyID: invoice.currency || 'TRY',
  ...
  private generateFileName(invoice: Invoice, ettn: string): string {
    ...
    return `${invoice.invoiceNumber}_${dateStr}_${ettn.substring(0, 8)}.xml`;
  }
