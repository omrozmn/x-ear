...
export class InvoiceService {
  ...
  async syncInvoice(tempId: string): Promise<Invoice> {
    ...
    const resp = await invoicesCreateInvoice(payload);
    if (resp && (resp as any).data && (resp as any).data.data) {
      const serverInv = (resp as any).data.data;
  ...
  async copyInvoice(id: string): Promise<Invoice> {
    ...
    try {
      ...
      if (resp && resp.data && resp.data.data) {
        const created = resp.data.data as Invoice;
        const invoices = await this.loadInvoices();
  ...
  async copyInvoiceWithCancellation(id: string): Promise<{ copy: Invoice; cancellation: Invoice }> {
    ...
    try {
      const resp = await invoicesCreateInvoice(cancelPayload);
      if (resp && resp.data && resp.data.data) {
        const cancellation = resp.data.data as Invoice;
        const invoices = await this.loadInvoices();
        invoices.unshift(cancellation);
        await this.saveInvoices(invoices);
  ...
  async getInvoices(filters?: InvoiceFilters): Promise<InvoiceSearchResult> {
    try {
      ...
      const total = typeof data.total === 'number' ? data.total : (data.meta?.total || items.length);
      const page = typeof data.page === 'number' ? data.page : (data.meta?.page || params.page);
      const limit = typeof data.limit === 'number' ? data.limit : (data.meta?.limit || params.limit);
  ...
  async issueInvoice(id: string): Promise<{ success: boolean; data?: any; error?: string }> {
    ...
    try {
      ...
      const resp = await fetch(`/api/invoices/${encodeURIComponent(String(serverId))}/issue`, {
        method: 'POST',
      ...
      if (!resp.ok) {
        ...
        return { success: false, error: `Issue failed: ${resp.status} ${errBody}` };
      }
      ...
      return { success: false, error: body && body.message ? body.message : 'Unknown error' };
    } catch (err) {
  ...
  async generateInvoicePdf(invoiceId: string): Promise<{ success: boolean; data?: Blob; error?: string }> {
    ...
    try {
      ...
      // Non-200 — try to read response body for diagnostics (response.data may be a Blob)
      let bodyText = '';
      try {
        const data = (response as any).data;
        if (data instanceof Blob) {
          bodyText = await new Response(data).text();
        ...
        bodyText = String(e instanceof Error ? e.message : e);
      }

      console.error('Invoice PDF generation returned non-200 status', (response as any).status, bodyText);
      return { success: false, error: `Server returned ${(response as any).status}: ${bodyText}` };
    } catch (error) {
  ...
  async generateSaleInvoicePdf(saleId: string): Promise<{ success: boolean; data?: Blob; error?: string }> {
    try {
      const response = await salesGenerateSaleInvoicePdf(saleId);
