...
export class InvoiceService {
  ...
  async syncInvoice(tempId: string): Promise<Invoice> {
    ...
    const resp = await api.invoicesCreateInvoice(payload);
  ...
  async copyInvoiceWithCancellation(id: string): Promise<{ copy: Invoice; cancellation: Invoice }> {
    ...
    try {
      const resp = await api.invoicesCreateInvoice(cancelPayload);
      if (resp && resp.data && resp.data.data) {
  ...
  async deleteInvoice(id: string): Promise<void> {
    ...
    try {
      await api.invoicesDeleteInvoice(String(serverId));
  ...
  async sendInvoice(id: string): Promise<Invoice> {
    ...
    const api = getXEarCRMAPIAutoGenerated();
    try {
      await api.invoicesSendToGib(id);
      // Try to refresh server state
      const resp = await api.invoicesGetInvoice(id);
      if (resp && resp.data && resp.data.data) {
  ...
  async generateInvoicePdf(invoiceId: string): Promise<{ success: boolean; data?: Blob; error?: string }> {
    ...
    try {
      ...
      const response = await api.invoicesGenerateInvoicePdf(String(serverId), { validateStatus: () => true });
