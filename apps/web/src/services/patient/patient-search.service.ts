...
export class PatientSearchService {
  ...
  private applyFilters(patients: Patient[], filters: PatientFilters): Patient[] {
    return patients.filter(patient => {
      ...
      // Label filter
      if (filters.label && patient.label !== filters.label) {
        return false;
      }
      ...
      if (filters.hasDevices !== undefined) {
        ...
        if (filters.hasDevices !== hasDevices) {
          return false;
        }
      }

      // SGK status filter
      if (filters.sgkStatus && patient.sgkStatus !== filters.sgkStatus) {
        return false;
      }

      // Priority score filter
      if (filters.priorityScore) {
        const score = patient.priorityScore || 0;
        if (filters.priorityScore.min !== undefined && score < filters.priorityScore.min) {
          return false;
        }
        if (filters.priorityScore.max !== undefined && score > filters.priorityScore.max) {
          return false;
        }
      }
  ...
  private calculatePatientMatch(request: PatientMatchRequest, patient: Patient): PatientMatchCandidate {
    ...
    if (request.name && patient.name) {
      const nameScore = this.calculateStringSimilarity(request.name, patient.name);
