...
import {
  ...
  SalesCreateSaleBody,
  SalesUpdateSaleBody
} from "../../api/generated/xEarCRMAPIAutoGenerated.schemas.ts/index";
...
import type { Patient as OrvalPatient } from '../../api/generated/xEarCRMAPIAutoGenerated.schemas.ts/index';
...
export class PatientApiService {
  ...
  private async _fetchAllPatientsInternal(perPage: number = 200): Promise<Patient[]> {
    ...
    try {
      ...
      const response = await api.patientsGetPatients({ page: 1, per_page: perPage });
  ...
  async createSale(patientId: string, saleData: unknown): Promise<ApiEnvelope<unknown>> {
    try {
      ...
      const api = getXEarCRMAPIAutoGenerated();
      const response = await api.salesCreateSale(body, {
        headers: {
  ...
  async createNote(patientId: string, noteData: unknown): Promise<unknown> {
    try {
      const response = await this.throttler.throttle(async () => {
        const api = getXEarCRMAPIAutoGenerated();
        return await api.patientSubresourcesCreatePatientNote(patientId, noteData as Record<string, unknown>);
      });
      return { 
        data: response.data, 
        success: true 
