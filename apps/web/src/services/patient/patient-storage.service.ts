import type { Patient as OrvalPatient } from '../../types/patient';
import { PatientFilters, PatientSearchResult } from '../../types/patient';
...
export class PatientStorageService {
   ...
   */
  async storePatients(patients: OrvalPatient[]): Promise<void> {
   ...
   */
  async getAllPatients(): Promise<OrvalPatient[]> {
   ...
   */
  async getPatientById(id: string): Promise<OrvalPatient | null> {
   ...
   */
  async updatePatient(patient: OrvalPatient): Promise<void> {
    const patients = await this.getAllPatients();
