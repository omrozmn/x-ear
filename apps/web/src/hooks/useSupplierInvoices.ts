...
export const useSupplierInvoices = (params: { supplierId: string } & SuppliersGetSupplierInvoicesParams) => {
  return useQuery({
    ...
    // queryFn: () => suppliersGetSupplierInvoices(params.supplierId, params),
    queryFn: async () => ({ data: [] }), // Placeholder
...
export const useAcceptSuggestedSupplier = () => {
  ...
  return useMutation({
    mutationFn: async (suggestedId: number | string) => {
      ...
      // return suppliersAcceptSuggestedSupplier(String(suggestedId), { headers: { 'Idempotency-Key': key } });
...
export const useRejectSuggestedSupplier = () => {
  ...
  return useMutation({
    mutationFn: async (suggestedId: number | string) => {
      ...
      // return suppliersRejectSuggestedSupplier(String(suggestedId));
      return Promise.resolve(); // Placeholder
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['suggested-suppliers'] });
