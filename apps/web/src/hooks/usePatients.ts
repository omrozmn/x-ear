...
import { Patient, PatientDevice, PatientFilters } from '../types/patient';
import { patientService } from '../services/patient.service';
import { patientSearchService, PatientSearchResult } from '../services/patient/patient-search.service';
...
export function usePatients(options: UsePatientsOptions = {}) {
  ...
  const searchPatients = useCallback(async (filters: SimpleCacheFilters) => {
    try {
      ...
      if (cacheEnabled && !filters.search) {
        ...
        const cacheResults = await patientCacheService.searchCachedPatients(filters);
        // Convert CacheSearchResult to UnifiedSearchResult
        unifiedResults = {
          patients: cacheResults.patients.map(item => ({
            id: item.id,
        ...
        // Cast patients to work with search service
        const searchResults = patientSearchService.searchPatients(patients as any, patientFilters as PatientFilters);
        // Convert PatientSearchResult to UnifiedSearchResult
        unifiedResults = {
          patients: searchResults.patients as unknown as Patient[],
  ...
  const createPatient = useCallback(async (patientData: Omit<Patient, 'id' | 'createdAt' | 'updatedAt'>): Promise<Patient> => {
    try {
      ...
      // Sync to server
      if (enableRealTimeSync && newPatient.id) {
