...
const CampaignsPage: React.FC = () => {
    ...
    const { data: creditData, isFetching: creditLoading } = useSmsGetCredit();
    ...
    const branchOptions = useMemo(() => {
        const items = branchesData?.data?.data ?? [];
        return items
    ...
    const { data: patientsData } = usePatientsGetPatients(
        { page: 1, per_page: 1 },
        { query: { enabled: mode === 'filters', refetchOnWindowFocus: false } }
    );
    const firstPatient = patientsData?.data?.data?.[0];
    ...
    const filterRecipientCount = patientsCountQuery.data?.data?.data?.count ?? 0;
