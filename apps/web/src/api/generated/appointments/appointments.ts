...
>(
  ...
  const queryKey =
    queryOptions?.queryKey ?? getAppointmentsListAppointmentsQueryKey(params);

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof appointmentsListAppointments>>
  > = ({ signal }) => appointmentsListAppointments(params, signal);
...
>(
  ...
  const queryOptions = getAppointmentsListAppointmentsQueryOptions(
    params,
    options,
  );
...
export const getAppointmentsGetAppointmentsQueryKey = (
  ...
  return [`/api/appointments/list`, ...(params ? [params] : [])] as const;
};
