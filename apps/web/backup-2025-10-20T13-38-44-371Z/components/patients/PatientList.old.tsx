...
import { usePatients } from '../../hooks/usePatients.old';
              ...
              <Button variant="primary" onClick={async () => {
                ...
                for (const p of selectedPatientsList) {
                  try {
                    ...
                    const labelValue = allowedLabels.includes(bulkTagText as any) ? (bulkTagText as Patient['label']) : undefined;
                    const updated = await updatePatient(p.id, { label: labelValue });
                    if (updated) success++;
                  } catch (err) {
