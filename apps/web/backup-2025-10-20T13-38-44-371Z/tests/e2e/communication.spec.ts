...
const testTemplate = {
  name: 'Test Şablonu',
  subject: 'Test Konusu',
  content: 'Merhaba {patientName}, randevunuz {appointmentDate} tarihinde planlanmıştır.',
  type: 'email' as const
};
...
test.describe('Communication Center E2E Tests', () => {
  ...
  test.describe('Message Composition and Sending', () => {
    test('should compose and send an email message', async ({ page }) => {
      ...
      await page.fill('[data-testid="recipient-search-input"]', testPatient.name);
      await page.click(`[data-testid="patient-${testPatient.name}"]`);
  ...
  test.describe('Template Management', () => {
    ...
    test('should edit an existing template', async ({ page }) => {
      ...
      const updatedContent = 'Güncellenen şablon içeriği: {patientName}';
