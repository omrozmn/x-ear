...
export const printerService = {
  async listPrinters(): Promise<Printer[]> {
    const { data } = await printerClient.get<Printer[]>('/api/v1/printers');
    return data;
  },

  async getPrinter(id: string): Promise<Printer> {
    const { data } = await printerClient.get<Printer>(`/api/v1/printers/${id}`);
    return data;
  },

  async createPrinter(input: CreatePrinterInput): Promise<Printer> {
    const { data } = await printerClient.post<Printer>('/api/v1/printers', input);
    return data;
  },

  async deletePrinter(id: string): Promise<void> {
    await printerClient.delete(`/api/v1/printers/${id}`);
  },

  async testPrinter(id: string): Promise<TestResult> {
    const { data } = await printerClient.post<TestResult>(`/api/v1/printers/${id}/test`);
    return data;
  },

  async getPrinterStatus(id: string): Promise<StatusResult> {
    const { data } = await printerClient.get<StatusResult>(`/api/v1/printers/${id}/status`);
    return data;
  },
  ...
  async listJobs(filters?: JobListFilters): Promise<JobListResult> {
    ...
    const { data } = await printerClient.get<JobListResult>('/api/v1/print/jobs', { params });
    return data;
  },

  async getJob(id: string): Promise<PrintJob> {
    const { data } = await printerClient.get<PrintJob>(`/api/v1/print/jobs/${id}`);
    return data;
  },
  ...
  async getJobStats(): Promise<JobStats> {
    const { data } = await printerClient.get<JobStats>('/api/v1/print/stats');
    return data;
  },
};
