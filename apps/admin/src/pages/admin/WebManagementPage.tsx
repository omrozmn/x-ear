    ...
    useEffect(() => {
        ...
        listWizardTemplates().then((data) => {
            if (!cancelled) {
                ...
                if (data.templates.length > 0) setSelectedTemplateKey(data.templates[0].key);
            }
        }).catch(() => {});
                ...
                <div className="rounded-3xl bg-white p-6 ring-1 ring-gray-200">
                    ...
                    <div className="mt-4 rounded-3xl bg-white p-4 ring-1 ring-gray-200">
                        ...
                        <div className="mt-3 grid gap-3">
                            ...
                            <div className="flex gap-2">
                                <button onClick={handleSavePageMeta} disabled={!selectedPage || busyKey === 'save-page'} className="flex-1 rounded-2xl bg-gray-900 px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-60">
                                    {busyKey === 'save-page' ? 'Kaydediliyor...' : 'Sayfa Bilgilerini Kaydet'}
                                </button>
                                {selectedPage && selectedPage.slug !== '/' && (
                                    <button onClick={() => handleDeletePage(selectedPage.slug)} disabled={busyKey === 'delete-page'} className="rounded-2xl bg-rose-50 px-4 py-2 text-sm font-medium text-rose-700 ring-1 ring-rose-200 disabled:cursor-not-allowed disabled:opacity-60">
