...
type TabId = 'blog' | 'web' | 'automation';
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Blog yazıları, web sitesi ve blog otomasyonu yönetimi</p>
      </div>
      ...
      <Tabs.Root value={activeTab} onValueChange={(v) => setActiveTab(v as TabId)}>
        <Tabs.List className="flex border-b border-gray-200 dark:border-gray-700 mb-6">
          ...
          <Tabs.Trigger value="automation" className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${activeTab === 'automation' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}>
            <Zap className="w-4 h-4" /> Blog Otomasyonu
          </Tabs.Trigger>
        ...
        <Tabs.Content value="automation"><Suspense fallback={<TabFallback />}><BlogAutomationPage /></Suspense></Tabs.Content>
