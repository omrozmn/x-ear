...
if not user:
    ...
    """, (hashed_password,))
    print(f"User e2etest created with role ADMIN")
else:
    cursor.execute("UPDATE users SET password_hash=?, role='ADMIN', is_active=1, is_phone_verified=1 WHERE username='e2etest'", (hashed_password,))
    print(f"User e2etest updated with role ADMIN")
