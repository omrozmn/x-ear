...
@given(
    ...
    recipient=st.emails(),
    scenario=st.sampled_from(["password_reset", "user_invite"])
)
@settings(max_examples=50, deadline=None)
def test_property_audit_log_creation(tenant_id, recipient, scenario):
    """
    ...
    mock_log = Mock(spec=SMTPEmailLog)
    mock_log.id = f"log_{tenant_id}"
    mock_log.tenant_id = tenant_id
    mock_log.recipient = recipient
    mock_log.scenario = scenario
    mock_log.status = "pending"
    
    with patch('services.email_service.SMTPEmailLog', return_value=mock_log):
        # Act
        email_log_id = email_service.queue_email(
            scenario=scenario,
            recipient=recipient,
    ...
    # Assert
    assert mock_log.tenant_id == tenant_id
    assert mock_log.recipient == recipient
    assert mock_log.scenario == scenario
    assert mock_log.status == "pending"
    mock_db.add.assert_called_once()
