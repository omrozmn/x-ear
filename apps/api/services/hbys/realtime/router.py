...
router = APIRouter(prefix="/hbys/notifications", tags=["HBYS - Notifications & Realtime"])
...
async def websocket_clinical_notifications(websocket: WebSocket):
    ...
    Connection flow:
      1. Client connects to ws://host/hbys/notifications/ws/clinical-notifications
    ...
    try:
        ...
        try:
            from middleware.unified_access import decode_and_validate_token
        ...
        # Re-register with manager (we already accepted above, so use internal registration)
        # We need to close and re-accept through manager, or just register directly
        ws_id = id(websocket)
        from .ws.connection_manager import ConnectionInfo
        info = ConnectionInfo(
            websocket=websocket,
            user_id=user_id,
            tenant_id=tenant_id,
...
def list_notifications(
    ...
    access: UnifiedAccess = Depends(require_access("hbys:notification:read")),
):
...
def get_notification(
    ...
    notif = service.get_notification(db, notification_id=notification_id, tenant_id=access.tenant_id)
    if not notif:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Notification '{notification_id}' not found.",
        )
    return ResponseEnvelope.create_success(data=NotificationRead.model_validate(notif))
...
def mark_read(
    ...
    notif = service.mark_notification_read(
        db, notification_id=notification_id, tenant_id=access.tenant_id, read_by=access.user_id
    )
    if not notif:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def mark_all_read(
    ...
    count = service.mark_all_read(db, tenant_id=access.tenant_id, user_id=access.user_id)
    return ResponseEnvelope.create_success(
...
async def send_notification(
    ...
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:notification:create")),
):
    ...
    notif, sent_count = await service.create_and_dispatch(
        db, data=create_data, tenant_id=access.tenant_id
    )
...
async def trigger_code_blue(
    ...
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:notification:code_blue")),
):
    ...
    return ResponseEnvelope.create_success(
        ...
        message=f"Code Blue broadcast to {sent_count} connection(s).",
    )
...
def list_preferences(
    ...
    result = service.list_preferences(db, user_id=access.user_id, tenant_id=access.tenant_id)
    return ResponseEnvelope.create_success(data=result)
...
def create_preference(
    ...
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:notification:update")),
):
    pref = service.upsert_preference(
        db, user_id=access.user_id, tenant_id=access.tenant_id, data=data
    )
    return ResponseEnvelope.create_success(
        data=PreferenceRead.model_validate(pref),
...
def update_preference(
    ...
    pref = service.update_preference(
        db, preference_id=preference_id, tenant_id=access.tenant_id, data=data
    )
    if not pref:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def delete_preference(
    preference_id: str,
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:notification:delete")),
):
    deleted = service.delete_preference(db, preference_id=preference_id, tenant_id=access.tenant_id)
    if not deleted:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Preference '{preference_id}' not found.",
        )
    return ResponseEnvelope.create_success(message="Preference deleted.")
