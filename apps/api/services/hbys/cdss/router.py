...
router = APIRouter(prefix="/hbys/cdss", tags=["HBYS - CDSS"])
...
def check_prescription(
    ...
    access: UnifiedAccess = Depends(require_access("hbys:cdss:check")),
):
    result = service.check_prescription(db, data=data, tenant_id=access.tenant_id)
...
def check_lab_result(
    ...
    result = service.check_lab_result(db, data=data, tenant_id=access.tenant_id)
    return ResponseEnvelope.create_success(data=result)
...
def check_duplicate_order(
    ...
    result = service.check_duplicate_order(db, data=data, tenant_id=access.tenant_id)
    return ResponseEnvelope.create_success(data=result)
...
def create_allergy(
    ...
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:allergy:create")),
):
    allergy = service.create_allergy(db, data=data, tenant_id=access.tenant_id)
    return ResponseEnvelope.create_success(
        data=PatientAllergyRead.model_validate(allergy),
        message="Alerji kaydı oluşturuldu.",
    )
...
def list_patient_allergies(
    ...
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:allergy:read")),
):
...
def get_allergy(
    ...
    allergy = service.get_allergy(db, allergy_id=allergy_id, tenant_id=access.tenant_id)
    if not allergy:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Alerji kaydı '{allergy_id}' bulunamadı.",
        )
    return ResponseEnvelope.create_success(data=PatientAllergyRead.model_validate(allergy))
...
def update_allergy(
    ...
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:allergy:update")),
):
    allergy = service.update_allergy(
        db, allergy_id=allergy_id, data=data, tenant_id=access.tenant_id
    )
    if not allergy:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    return ResponseEnvelope.create_success(
        data=PatientAllergyRead.model_validate(allergy),
        message="Alerji kaydı güncellendi.",
    )
...
def delete_allergy(
    allergy_id: str,
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:allergy:delete")),
):
    deleted = service.delete_allergy(db, allergy_id=allergy_id, tenant_id=access.tenant_id)
    if not deleted:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Alerji kaydı '{allergy_id}' bulunamadı.",
        )
    return ResponseEnvelope.create_success(message="Alerji kaydı silindi.")
...
def list_alerts(
    ...
    limit: int = Query(50, ge=1, le=200),
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:alert:read")),
):
...
def get_alert(
    ...
    alert = service.get_alert(db, alert_id=alert_id, tenant_id=access.tenant_id)
    if not alert:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Uyarı '{alert_id}' bulunamadı.",
        )
    return ResponseEnvelope.create_success(data=ClinicalAlertRead.model_validate(alert))
...
def acknowledge_alert(
    ...
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:alert:acknowledge")),
):
    ...
    return ResponseEnvelope.create_success(
        data=ClinicalAlertRead.model_validate(alert),
...
def override_alert(
    ...
    data: AlertOverride,
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:alert:override")),
):
    ...
    return ResponseEnvelope.create_success(
        data=ClinicalAlertRead.model_validate(alert),
        message="Uyarı geçersiz kılındı.",
    )
...
def create_protocol(
    ...
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:protocol:create")),
):
    protocol = service.create_protocol(db, data=data)
    return ResponseEnvelope.create_success(
        data=ClinicalProtocolRead.model_validate(protocol),
        message="Klinik protokol oluşturuldu.",
    )
...
def get_protocol(
    ...
    return ResponseEnvelope.create_success(data=ClinicalProtocolRead.model_validate(protocol))
...
def update_protocol(
    ...
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:protocol:update")),
):
    ...
    return ResponseEnvelope.create_success(
        data=ClinicalProtocolRead.model_validate(protocol),
        message="Klinik protokol güncellendi.",
    )
...
def delete_protocol(
    protocol_id: str,
    db: Session = Depends(get_db),
    access: UnifiedAccess = Depends(require_access("hbys:cdss:protocol:delete")),
):
    ...
    return ResponseEnvelope.create_success(message="Klinik protokol silindi.")
