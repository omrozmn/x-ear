"use strict";
/**
 * Generated API Client - Placeholder Implementation
 * This file serves as a placeholder until the actual client generation is resolved.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.PatientNoteApi = exports.HearingTestApi = exports.DeviceApi = exports.AppointmentApi = exports.PatientApi = void 0;
// API Classes - Placeholder implementations
class PatientApi {
    constructor(configuration) {
        this.configuration = configuration;
    }
    async getPatients(params) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async getPatient(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async createPatient(patient) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async updatePatient(id, patient) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async deletePatient(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
}
exports.PatientApi = PatientApi;
class AppointmentApi {
    constructor(configuration) {
        this.configuration = configuration;
    }
    async getAppointments(params) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async getAppointment(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async createAppointment(appointment) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async updateAppointment(id, appointment) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async deleteAppointment(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
}
exports.AppointmentApi = AppointmentApi;
class DeviceApi {
    constructor(configuration) {
        this.configuration = configuration;
    }
    async getDevices(params) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async getDevice(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async createDevice(device) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async updateDevice(id, device) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async deleteDevice(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
}
exports.DeviceApi = DeviceApi;
class HearingTestApi {
    constructor(configuration) {
        this.configuration = configuration;
    }
    async getHearingTests(params) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async getHearingTest(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async createHearingTest(test) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async updateHearingTest(id, test) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async deleteHearingTest(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
}
exports.HearingTestApi = HearingTestApi;
class PatientNoteApi {
    constructor(configuration) {
        this.configuration = configuration;
    }
    async getPatientNotes(patientId) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async getPatientNote(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async createPatientNote(note) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async updatePatientNote(id, note) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
    async deletePatientNote(id) {
        throw new Error('Generated client not yet implemented. Use legacy client.');
    }
}
exports.PatientNoteApi = PatientNoteApi;
// Default export with all APIs
exports.default = {
    PatientApi,
    AppointmentApi,
    DeviceApi,
    HearingTestApi,
    PatientNoteApi,
};
