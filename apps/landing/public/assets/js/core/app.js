...
const PrioritySystem = {
  ...
  autoTagHighPriorityPatients() {
    ...
    highPriorityPatients.forEach(patient => {
      // Ensure tags is an array (handle malformed formats)
...
const PatientManager = {
  ...
  updatePatient(id, updates) {
    ...
    if (index !== -1) {
      ...
      // Check if this is sample data (p1, p2, p3) - don't make API calls for these
      const isSampleData = ['p1', 'p2', 'p3'].includes(id);

      // If API is available and this is not sample data, prefer calling backend so central DB is authoritative
      if (!isSampleData && typeof window !== 'undefined' && window.APIConfig && typeof window.APIConfig.makeRequest === 'function') {
        try {
