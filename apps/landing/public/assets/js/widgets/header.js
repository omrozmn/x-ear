...
class HeaderWidget {
            ...
            <div class="relative flex items-center space-x-3 border-l border-gray-200 pl-4">
                ...
                <div id="profile-dropdown" role="menu" aria-orientation="vertical" aria-labelledby="profile-btn" class="hidden absolute right-0 top-full mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 z-50 origin-top-right py-1 divide-y divide-gray-100">
                    <div class="py-1">
                        ...
                        <a href="/settings.html#users" id="crmAdminLink" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hidden">CRM Yönetimi</a>
                        <a href="/admin-panel/" id="adminPanelLink" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hidden">Owner Admin</a>
                    </div>
                    <div class="py-1">
    ...
    initialize() {
        ...
        if (darkModeToggle) {
            ...
            darkModeToggle.addEventListener('click', () => {
                try {
                    if (window.DarkMode) {
                        if (typeof window.DarkMode.toggleDarkMode === 'function') {
                            window.DarkMode.toggleDarkMode();
                        } else if (typeof window.DarkMode.toggle === 'function') {
                            window.DarkMode.toggle();
                        } else if (typeof window.DarkMode.enable === 'function' && typeof window.DarkMode.disable === 'function') {
                            // fallback: flip based on current state
                            const isDark = (typeof window.DarkMode.isDarkMode === 'function') ? window.DarkMode.isDarkMode() : (typeof window.DarkMode.isDark === 'function' ? window.DarkMode.isDark() : document.documentElement.classList.contains('dark-mode'));
                            if (isDark) window.DarkMode.disable(); else window.DarkMode.enable();
                        }
                    }
                } catch (err) {
                    console.warn('Dark mode toggle handler failed', err);
                }
        ...
        // Proactively show Admin Panel link if user has access; fetch dynamic URL from server config
        (async ()=>{
            try{
                ...
                try {
                    const resp = await fetch(configEndpoint);
                    if (resp.ok) {
                        const b = await resp.json().catch(()=>null);
                        if (b && b.data && b.data.data && b.data.data.adminPanelUrl) {
                            adminUrl = b.data.data.adminPanelUrl;
                            configFound = true;
                        }
                    }
                } catch (err) {
                    console.log(`Config endpoint ${configEndpoint} not available:`, err.message);
                    // Use default admin URL if config is not available
                }
                
                if (adminLink){ adminLink.setAttribute('href', adminUrl); }
                // CRM Admin link: show for global CRM admins (role==admin) or users with users:manage global permission
                try{
                    const crmLink = document.getElementById('crmAdminLink');
                    if (crmLink && window.authManager && (window.authManager.is('admin') || window.authManager.can('users:manage'))){
                        crmLink.classList.remove('hidden');
                    }
                }catch(e){}
                // Owner Admin link: app owner/external admin panel
                try{
                    if (adminLink && window.authManager && typeof window.authManager.can === 'function' && window.authManager.can('admin-panel:access')){
         ...
         if (profileLink) {
            profileLink.addEventListener('click', (e) => {
                ...
                try {
                    // show admin panel link if allowed
                    try{ if (window.authManager && typeof window.authManager.can === 'function' && window.authManager.can('admin-panel:access')){ document.getElementById('adminPanelLink').classList.remove('hidden'); } }catch(e){}
    ...
    updateDarkModeToggle() {
        ...
        try {
            if (window.DarkMode) {
                if (typeof window.DarkMode.isDarkMode === 'function') isDarkMode = window.DarkMode.isDarkMode();
                else if (typeof window.DarkMode.isDark === 'function') isDarkMode = window.DarkMode.isDark();
                else isDarkMode = document.documentElement.classList.contains('dark-mode');
            } else {
