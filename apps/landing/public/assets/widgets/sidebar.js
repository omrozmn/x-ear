...
class SidebarWidget {
    constructor(activePage = '') {
        ...
        this.isCollapsed = localStorage.getItem(window.STORAGE_KEYS?.SIDEBAR_COLLAPSED || 'sidebarCollapsed') === 'true';
    ...
    renderSGKSubmenu() {
        ...
        const isExpanded = isActive || localStorage.getItem(window.STORAGE_KEYS?.SGK_SUBMENU_EXPANDED || 'sgkSubmenuExpanded') === 'true';
    ...
    renderFaturaSubmenu() {
        ...
        const isExpanded = isActive || localStorage.getItem(window.STORAGE_KEYS?.FATURA_SUBMENU_EXPANDED || 'faturaSubmenuExpanded') === 'true';
        const activeClass = isActive ? ' active' : '';
    ...
    renderReportsSubmenu() {
        ...
        const isExpanded = isActive || localStorage.getItem(window.STORAGE_KEYS?.REPORTS_SUBMENU_EXPANDED || 'reportsSubmenuExpanded') === 'true';
        const activeClass = isActive ? ' active' : '';
...
window.toggleSidebar = function() {
    ...
    if (sidebar) {
        ...
        localStorage.setItem(window.STORAGE_KEYS?.SIDEBAR_COLLAPSED || 'sidebarCollapsed', isCollapsed);
...
window.toggleSGKSubmenu = function() {
    ...
    if (isCollapsed) {
        ...
        if (collapsedSubmenuIcons) {
            ...
            localStorage.setItem(window.STORAGE_KEYS?.SGK_SUBMENU_EXPANDED_COLLAPSED || 'sgkSubmenuExpandedCollapsed', !isVisible ? 'true' : 'false');
        }
    } else {
        ...
        if (submenu && arrow) {
            ...
            if (isExpanded) {
                ...
                localStorage.setItem(window.STORAGE_KEYS?.SGK_SUBMENU_EXPANDED || 'sgkSubmenuExpanded', 'false');
            } else {
                submenu.classList.add('expanded');
                arrow.classList.add('expanded');
                localStorage.setItem(window.STORAGE_KEYS?.SGK_SUBMENU_EXPANDED || 'sgkSubmenuExpanded', 'true');
            }
        }
    }
};
...
window.toggleFaturaSubmenu = function() {
    ...
    if (isCollapsed) {
        ...
        if (collapsedSubmenuIcons) {
            const isVisible = collapsedSubmenuIcons.style.display === 'flex';
            collapsedSubmenuIcons.style.display = isVisible ? 'none' : 'flex';
            localStorage.setItem(window.STORAGE_KEYS?.FATURA_SUBMENU_EXPANDED_COLLAPSED || 'faturaSubmenuExpandedCollapsed', !isVisible ? 'true' : 'false');
        }
    } else {
        // Normal submenu toggle behavior when not collapsed
        if (submenu && arrow) {
            ...
            if (isExpanded) {
                submenu.classList.remove('expanded');
                arrow.classList.remove('expanded');
                localStorage.setItem(window.STORAGE_KEYS?.FATURA_SUBMENU_EXPANDED || 'faturaSubmenuExpanded', 'false');
            } else {
                submenu.classList.add('expanded');
                arrow.classList.add('expanded');
                localStorage.setItem(window.STORAGE_KEYS?.FATURA_SUBMENU_EXPANDED || 'faturaSubmenuExpanded', 'true');
            }
        }
    }
};
...
window.toggleReportsSubmenu = function() {
    ...
    if (isCollapsed) {
        ...
        if (collapsedSubmenuIcons) {
            const isVisible = collapsedSubmenuIcons.style.display === 'flex';
            collapsedSubmenuIcons.style.display = isVisible ? 'none' : 'flex';
            localStorage.setItem(window.STORAGE_KEYS?.REPORTS_SUBMENU_EXPANDED_COLLAPSED || 'reportsSubmenuExpandedCollapsed', !isVisible ? 'true' : 'false');
        }
    } else {
        // Normal submenu toggle behavior when not collapsed
        if (submenu && arrow) {
            ...
            if (isExpanded) {
                submenu.classList.remove('expanded');
                arrow.classList.remove('expanded');
                localStorage.setItem(window.STORAGE_KEYS?.REPORTS_SUBMENU_EXPANDED || 'reportsSubmenuExpanded', 'false');
            } else {
                submenu.classList.add('expanded');
                arrow.classList.add('expanded');
                localStorage.setItem(window.STORAGE_KEYS?.REPORTS_SUBMENU_EXPANDED || 'reportsSubmenuExpanded', 'true');
            }
        }
    }
};
