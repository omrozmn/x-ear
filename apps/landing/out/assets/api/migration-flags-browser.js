// Migration Feature Flags - Browser Compatible Version
...
function setMigrationFlags(flags) {
    ...
    try {
        ...
        console.log('✅ Migration flags updated:', updated);
    } catch (error) {
...
if (typeof window !== 'undefined') {
    window.getMigrationFlags = getMigrationFlags;
    window.setMigrationFlags = setMigrationFlags;
    window.setMigrationPhase = setMigrationPhase;
    window.emergencyRollback = emergencyRollback;
    window.shouldUseGeneratedClient = shouldUseGeneratedClient;
    window.MIGRATION_PHASES = MIGRATION_PHASES;
}