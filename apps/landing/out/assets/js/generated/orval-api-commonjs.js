(function() {
  ...
  API_FUNCTIONS.forEach(funcName => {
    window[funcName] = async function(...args) {
      ...
      let method = 'GET';
      let path = '';
      let bodyArgIndex = 0; // default: first arg is body for non-GET
      
      // Special handling for inventory functions
      if (funcName === 'inventoryGetInventoryStats') {
        ...
        console.log(`🎯 Special handling for ${funcName}: ${method} ${path}`);
      } else if (funcName === 'inventoryCreateInventoryItem') {
        method = 'POST';
        path = 'inventory';
        console.log(`🎯 Special handling for ${funcName}: ${method} ${path}`);
      } else if (funcName === 'inventoryDeleteInventoryItem') {
        method = 'DELETE';
        // First argument should be the item ID
        const itemId = args[0];
        path = `inventory/${itemId}`;
        console.log(`🎯 Special handling for ${funcName}: ${method} ${path}`);
      } else if (funcName === 'salesGetPatientSales') {
        // GET /api/patients/{patientId}/sales
        method = 'GET';
        const patientId = args[0];
        path = `patients/${patientId}/sales`;
        console.log(`🎯 Special handling for ${funcName}: ${method} ${path}`);
      } else if (funcName === 'documentsAddPatientDocument') {
        // POST /api/patients/{patientId}/documents (body is second arg)
        method = 'POST';
        const patientId = args[0];
        path = `patients/${patientId}/documents`;
        bodyArgIndex = 1;
        console.log(`🎯 Special handling for ${funcName}: ${method} ${path}`);
      } else if (funcName === 'paymentsCreatePromissoryNotes') {
      ...
      try {
        ...
        const response = await fetch(fullUrl, {
          ...
          body: method !== 'GET' && args.length > bodyArgIndex ? JSON.stringify(args[bodyArgIndex]) : undefined
        });
