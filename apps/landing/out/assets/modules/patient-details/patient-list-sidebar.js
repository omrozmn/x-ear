class PatientListSidebarComponent {
    constructor(apiClient) {
        ...
        this.isCollapsed = localStorage.getItem('patientListCollapsed') === 'true';
        this.searchQuery = '';
        this.patients = [];
    ...
    setupEventListeners() {
        ...
        // Immediately position patient list sidebar based on main sidebar state
        const mainSidebar = document.querySelector('.sidebar-nav');
        const mainSidebarWidth = (mainSidebar && mainSidebar.classList.contains('collapsed')) ? 80 : 240;
        if (container) {
            container.style.left = `${mainSidebarWidth}px`;
            console.log(`🔧 Initial patient list sidebar positioned at ${mainSidebarWidth}px`);
        }
    ...
    /**
     * Adjust layout based on sidebar states - dynamically position patient list sidebar
     */
    updateLayoutMargins() {
        const sidebar = document.querySelector('.sidebar-nav');
        const patientSidebarContainer = document.getElementById('patient-list-sidebar-container');
        ...
        // Calculate main sidebar width dynamically
        const mainSidebarWidth = sidebarCollapsed ? 80 : 240;
        const patientListWidth = 320;

        console.log('Layout update:', { sidebarCollapsed, patientListCollapsed, mainSidebarWidth });
        ...
        // Position patient list sidebar dynamically to stick to main sidebar
        patientSidebarContainer.style.left = `${mainSidebarWidth}px`;
        console.log(`🔧 Patient list sidebar positioned at ${mainSidebarWidth}px (sticking to main sidebar)`);

        // Apply classes for other layout adjustments
        if (sidebarCollapsed) {
        ...
        // Calculate and set main content margin dynamically
        const totalSidebarWidth = patientListCollapsed ? mainSidebarWidth : (mainSidebarWidth + patientListWidth);
        mainContent.style.marginLeft = `${totalSidebarWidth}px`;
        console.log(`🔧 Main content margin set to ${totalSidebarWidth}px`);
        ...
        if (patientListCollapsed) {
            ...
            if (toggleButton) {
                toggleButton.style.display = 'block';
                // Position toggle button dynamically after main sidebar
                toggleButton.style.left = `${mainSidebarWidth + 10}px`;
            }
        } else {
