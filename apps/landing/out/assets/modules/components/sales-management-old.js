class SalesManagementComponent {
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Satış No</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ${this.getPatientSales(patientId).map(sale => `
              <tr>
                <td class="px-4 py-3 text-sm text-gray-900">${new Date(sale.date).toLocaleDateString('tr-TR')}</td>
                <td class="px-4 py-3 text-sm font-medium text-gray-900">${sale.id}</td>
                <td class="px-4 py-3 text-sm text-gray-600">
                  ${sale.devices.map(d => `${d.brand} ${d.model} (${d.ear === 'left' ? 'Sol' : d.ear === 'right' ? 'Sağ' : 'Her İki'})`).join(', ')}
                </td>
                <td class="px-4 py-3 text-sm text-right font-semibold text-gray-900">${(sale.totalAmount || 0).toLocaleString('tr-TR')} TL</td>
  ...
  generateInvoice(sale) {
    const invoice = {
      ...
      items: sale.items.reduce((sum, item) => sum + (item.quantity * item.price), 0),
...
window.SalesManagementComponent = SalesManagementComponent;-2 text-sm text-gray-900">${item.name}</td>
