...
const {query: queryOptions} = options ?? {};
  ...
  const queryKey =  queryOptions?.queryKey ?? getPermissionsGetRolePermissionsQueryKey(roleName);
   ...
   return  { queryKey, queryFn, enabled: !!(roleName), ...queryOptions} as UseQueryOptions<Awaited<ReturnType<typeof permissionsGetRolePermissions>>, TError, TData> & { queryKey: DataTag<QueryKey, TData, TError> }
}
