...
class EmergencyVisit(BaseModel, TenantScopedMixin):
    __tablename__ = "emergency_visits"

    id = Column(String(36), primary_key=True, default=lambda: gen_id("emr"))

    # Patient reference (logical FK - no constraint to avoid cross-service coupling)
    ...
    triage_nurse_id = Column(String(36), nullable=True)
    triage_date = Column(DateTime, nullable=True)

    # Doctor assignment (logical FK)
