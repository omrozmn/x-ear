...
async def create_checkout_session(data: CheckoutSession, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"success": True, "checkoutUrl": f"/checkout/success?payment_id={payment.id}", "paymentId": payment.id}
...
async def confirm_payment(data: PaymentConfirm, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.commit()
        return {"success": True, "message": "Payment confirmed and tenant activated"}
    except HTTPException:
        raise
    except Exception as e:
        db.rollback()
