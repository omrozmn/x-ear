...
TOKEN=$(python3 get_token.py 2>/dev/null | tail -1)
...
SALE_ID=$(echo "$SALE_RESPONSE" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['id'])")
...
DEVICE1_ID=$(echo "$SALE_DATA" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['deviceAssignments'][0]['id'])")
DEVICE2_ID=$(echo "$SALE_DATA" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['deviceAssignments'][1]['id'])")
DEVICE1_PRICE=$(echo "$SALE_DATA" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['deviceAssignments'][0]['listPrice'])")
DEVICE2_PRICE=$(echo "$SALE_DATA" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['deviceAssignments'][1]['listPrice'])")
...
NEW_DEVICE1_PRICE=$(echo "$SALE_DATA" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['deviceAssignments'][0]['listPrice'])")
NEW_DEVICE2_PRICE=$(echo "$SALE_DATA" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['deviceAssignments'][1]['listPrice'])")

echo "After update (from sale endpoint):"
...
DEVICE1_DIRECT=$(echo "$DEVICE1_DATA" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['listPrice'])")
DEVICE2_DIRECT=$(echo "$DEVICE2_DATA" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['listPrice'])")

echo ""
echo "After update (from device assignment endpoint):"
