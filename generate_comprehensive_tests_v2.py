...
def generate_test_data(suffix: str) -> Dict[str, Any]:
    ...
    # Use timestamp + random for truly unique values
    ...
    return {
        ...
        "INVOICE": {
            ...
            "recipientEmail": "invoice@test.com",
...
def generate_script():
    ...
    # Use full timestamp + random for absolute uniqueness
...
ITEM_DATA='{json.dumps(test_data["ITEM"])}'
...
ADDON_DATA='{json.dumps(test_data["ADDON"])}'
...
ROLE_DATA='{json.dumps(test_data["ROLE"])}'
...
ASSIGNMENT_DATA='{json.dumps(test_data["ASSIGNMENT"])}'
...
APPT_DATA='{json.dumps(test_data["APPOINTMENT"])}'
...
NOTIF_DATA='{json.dumps(test_data["NOTIFICATION"])}'
...
TEMPLATE_DATA='{json.dumps(test_data["TEMPLATE"])}'
...
INVOICE_DATA='{json.dumps(test_data["INVOICE"])}'
...
PACKAGE_DATA='{json.dumps(test_data["PACKAGE"])}'
...
INTEGRATION_DATA='{json.dumps(test_data["INTEGRATION"])}'
...
SMS_HEADER_DATA='{json.dumps(test_data["SMS_HEADER"])}'
    ...
    for category in sorted(categories.keys()):
        ...
        for ep in categories[category]:
            ...
            clean_path = path\
                ...
                .replace('{addon_id}', '${ADDON_ID:-null}')\
                .replace('{role_id}', '${ROLE_ID:-null}')\
                .replace('{assignment_id}', '${ASSIGNMENT_ID:-null}')\
                .replace('{appointment_id}', '${APPT_ID:-null}')\
                .replace('{notification_id}', '${NOTIF_ID:-null}')\
                .replace('{template_id}', '${TEMPLATE_ID:-null}')\
                .replace('{invoice_id}', '${INVOICE_ID:-null}')\
                .replace('{package_id}', '${PACKAGE_ID:-null}')\
                .replace('{integration_id}', '${INTEGRATION_ID:-null}')\
                .replace('{header_id}', '${HEADER_ID:-null}')\
