...
function useFuzzySearch(initialData, options = {}) {
  ...
  const removeItems = useCallback((predicate) => {
    setDataState((prevData) => prevData.filter((item) => !predicate(item)));
  }, []);
  const stats = useMemo(() => fuzzySearch2.getStats(), [fuzzySearch2]);
  return {
    results,
    query,
    setQuery,
...
function useFileUpload(options = {}) {
  ...
  const parseOptionsRef = useRef(opts.parseOptions || {});
  const conversionOptionsRef = useRef(opts.conversionOptions || {});
  ...
  const validateFile = useCallback2((file) => {
  ...
  const uploadFile = useCallback2(async (file) => {
  ...
  const processFile = useCallback2(async () => {
    if (!state.file) {
  ...
  const convertColumn = useCallback2(async (columnIndex, targetType) => {
  ...
  const convertAllColumns = useCallback2(async () => {
...
var calculateAge = (birthDate) => {
  try {
    ...
    return differenceInYears(/* @__PURE__ */ new Date(), birth);
  } catch {
    return 0;
  }
};
